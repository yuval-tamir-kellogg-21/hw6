...
// MY API KEY: 2be4ae316103e214a4775ef9833e8ecd
...
window.addEventListener('DOMContentLoaded', async function(event) {
  ...
  console.log(`Number to movies in collection: ${querySnapshot.size}`)
  let existingMovies = querySnapshot.docs
  // console.log(existingMovies)
  for(let j=0; j<existingMovies.length; j++){
    let eMovies = existingMovies[j].data()
    console.log(eMovies.id)
  

  for(i=0; i<movies.results.length; i++) {
    let posterPath = movies.results[i].poster_path
    ...
    document.querySelector('.movies').insertAdjacentHTML('beforeend', `
      ...
      `)
  document.querySelector(`.movie-${eMovies.id}`).classList.add('opacity-20')
  ...
  // ⬇️ ⬇️ ⬇️
    document.querySelector(`.watched-button-${movieId}`).addEventListener('click', async function(event) {
      ...
      // console.log(`Movie ${movieId} has been clicked`)
